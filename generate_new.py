...
configPath = os.path.join(os.path.dirname(os.path.abspath(__file__)), "config.yaml")
...
class policy:
    ...
    def __init__(self, name, type):
        ...
        self.CUSTOMER_BAN_DIRS = config['banner']['banPath'].split(',') if config['banner']['banPath'] != None else []
        self.CUSTOMER_BAN_TYPES = config['banner']['banType'].split(',') if config['banner']['banType'] != None else []

        self.CUSTOMER_MUST_TYPES = ['user_home_t']
...
#                   line = "corenet_udp_bind_%s(%s_t)\n" % (port_name, self.name)
    ...
    def __find_path(self, file):
        ...
        flag = False
        for d in self.CUSTOMER_BAN_DIRS:
            if file.find(d) == 0:
                flag = True
                break
        if flag or default_type in self.CUSTOMER_BAN_TYPES:
            """判断是否在已知模板中"""
            print(f"{file} 在禁止名单中")
            for d in self.DEFAULT_DIRS:
                if file.find(d) == 0:
                    """
                    是的话，如果不在CUSTOMER_DIRS中，则为硬件资源，使用默认模板（使用原本type）
                    不是的话，使用自定义模板（使用原本type）
                    """
                    if d not in self.CUSTOMER_DIRS :
                        print(f"{file} 在硬件资源模板中")
                        self.DEFAULT_DIRS[d][1].append(file)
                        print(self.DEFAULT_DIRS[d])
                        return self.DEFAULT_DIRS[d]
                    else:
                        print(f"{file} 不在硬件资源模板中，使用自定义对应模板")
            ...
            """若不在，则使用custom模板，使用默认type"""
            print(f"{file} 不在资源模板中，使用默认自定义模板")
            self.CUSTOMER_DIRS[file] = [default_type, [], customer]
            self.CUSTOMER_DIRS[file][1].append(file)
            if default_type not in self.CUSTOMER_KEYS:
                self.CUSTOMER_KEYS.append(file)
        ...
        """判断是否在默认type中"""
        print(f"{file} 不在禁止名单中")
        for d in self.DEFAULT_DIRS:
            if file.find(d) == 0:
                print(f"{file} 在已知模板中")
                owner_name, owner_id = self.get_file_owner(file)
                if d not in self.CUSTOMER_DIRS :
                    print(f"{file} 为硬件文件，使用默认type和对应模板")
                    self.DEFAULT_DIRS[d][1].append(file)
                    print(self.DEFAULT_DIRS[d])
                    return self.DEFAULT_DIRS[d]
                elif owner_name == 'root' or owner_id == 0:
                    print(f"{file} 为系统文件，使用自定义对应模板")
                    self.CUSTOMER_DIRS[file] = [default_type, [], customer]
                    self.CUSTOMER_DIRS[file][1].append(file)
                    if default_type not in self.CUSTOMER_KEYS:
                        self.CUSTOMER_KEYS.append(file)
                    ...
                    print(f"{file} 为系统文件，使用对应小标签模板")
                    self.CUSTOMER_DIRS[d][1].append(file)
                    print(self.CUSTOMER_DIRS[d])
                    return self.CUSTOMER_DIRS[d]

        print(f"{file} 不在已知模板中")
        if default_type != 'unconfined' and default_type not in self.CUSTOMER_MUST_TYPES:
            print(f"{file} 不在宽泛权限名单中，使用默认标签")
            self.CUSTOMER_DIRS[file] = [default_type, [], customer]
            self.CUSTOMER_DIRS[file][1].append(file)
            if default_type not in self.CUSTOMER_KEYS:
                self.CUSTOMER_KEYS.append(file)
...
"""
    ...
    def generate_if(self):
        ...
        for d in self.CUSTOMER_KEYS:
            if len(self.CUSTOMER_DIRS[d][1]) > 0:
                ...
                t2 = re.sub("CUSTOMTYPE", self.CUSTOMER_DIRS[d][0], t1)
                newif += t2
    ...
    def generate_sh(self):
        ...
        platformName = platform.freedesktop_os_release().get('NAME')
        platformName = platformName.split(' ')[0].lower()

        if (platformName in ("redhat", "centos", "suse", "fedora", "mandrake", "mandriva")):
